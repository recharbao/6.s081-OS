...
// debug: pay attention to here, >> first instead of /16 !
...
int bit_flip_isset(char *array, int index) {
  // char b = array[index/16];
  // char m = (1 << (index % 16));
  // return m == (b & m);
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    if (bit_flip_isset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
int
bd_initfree_pair(int k, int bi, void* lbound, void* rbound) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bit_flip_isset(bd_sizes[k].alloc, bi)) {
    // one of the pair is free
    free = BLK_SIZE(k);
    if(addr(k, buddy) >= lbound && addr(k, buddy) < rbound)
...
int
bd_initfree(void *bd_left, void *bd_right, void* lbound, void* rbound) {
  int free = 0;

  for (int k = 0; k < MAXSIZE; k++) {   // skip max size
    int left = blk_index_next(k, bd_left);
    int right = blk_index(k, bd_right);
    free += bd_initfree_pair(k, left, lbound, rbound);
    if(right <= left)
      continue;
    free += bd_initfree_pair(k, right, lbound, rbound);
  }
  return free;
}
